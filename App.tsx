...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen relative font-mono text-sm">
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setShowSettings(false)}>
          <div className="bg-black border border-theme p-6 max-w-sm w-full shadow-[0_0_20px_rgba(0,0,0,0.8)] relative" onClick={e => e.stopPropagation()}>
            <button onClick={() => setShowSettings(false)} className="absolute top-2 right-2 text-theme hover:text-white">
              <X className="w-5 h-5" />
            </button>
              ...
              <div className="grid grid-cols-1 gap-2">
                {THEMES.map((theme) => (
                  <button
                    ...
                    className={`flex items-center gap-3 px-4 py-2 border text-left transition-all ${
      ...
      <header className="border-b border-theme-dim sticky top-0 z-50 bg-black/90 backdrop-blur-md">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div 
             ...
             onClick={goHome}
          >
             <div className="w-8 h-8 border border-theme flex items-center justify-center group-hover:bg-theme group-hover:text-black transition-colors">
          ...
          <div className="flex items-center gap-6">
             ...
             >
                <Globe className="w-4 h-4" /> [GLOBAL_DATA]
             </button>
             {state.view !== 'HOME' && (
               <button 
                  onClick={goHome}
             ...
             <button
                ...
                className="text-theme-dim hover:text-theme transition-colors"
      ...
      <main className="container mx-auto px-4 py-8 relative z-10 crt-flicker">
        ...
        {state.view === 'HOME' && (
           <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
              <h1 className="text-4xl md:text-6xl font-bold text-center mb-4 glow-text tracking-tighter">
                DOTA_TERMINAL
              </h1>
              ...
              <div className="mt-12 flex gap-4">
                  <button onClick={navigateToGlobal} className="hover-bg-theme border border-theme text-theme px-6 py-3 font-bold transition-all uppercase tracking-widest text-xs">
                      [BROWSE_HEROES]
                  </button>
                  <button onClick={() => navigateToMatch(7562624925)} className="hover-bg-theme-dim border border-theme-dim text-theme-dim px-6 py-3 font-bold transition-all uppercase tracking-widest text-xs hover:text-theme hover:border-theme">
                      [SAMPLE_MATCH]
                  </button>
              </div>
           </div>
      ...
      <footer className="border-t border-theme-dim py-8 text-center text-theme-dim text-xs uppercase tracking-widest opacity-50 relative z-10">
