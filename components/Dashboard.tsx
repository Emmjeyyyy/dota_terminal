...
const Dashboard: React.FC<DashboardProps> = ({ profile, matches, onMatchClick }) => {
  ...
  return (
    <div className="space-y-8">
      {/* Header Profile */}
      <div className="flex items-center gap-6 terminal-box p-6 relative overflow-hidden">
        ...
        <div className="z-10">
          <h1 className="text-3xl font-bold text-theme uppercase tracking-tight glow-text">{profile.profile.personaname}</h1>
          <a 
            href={profile.profile.profileurl} 
            target="_blank" 
            rel="noreferrer" 
            className="text-xs text-theme-dim hover:text-theme border-b border-dashed border-theme-dim pb-0.5 uppercase tracking-wider"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
           <div className="terminal-box p-5 flex flex-col justify-between group">
              <div className="flex items-start justify-between">
                <div>
                   <p className="text-theme-dim text-[10px] uppercase tracking-widest mb-1">OPTIMAL_SQUAD_DETECTED</p>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                 {bestCombo.teammates.map((mate) => (
                    <div key={mate.account_id} className="flex items-center gap-1 border border-theme-dim text-theme-dim px-2 py-1 text-xs hover:border-theme hover:text-theme transition-colors cursor-default">
                       ...
                       <span className="uppercase">{mate.personaname}</span>
                    </div>
                 ))}
              </div>
           ...
           <div className="md:col-span-2 terminal-box p-5 flex items-center justify-center text-theme-dim text-sm font-mono border-l-4 border-l-theme">
              {bestCombo.id === 'SOLO' ? (
                <div className="flex items-center gap-3">
                   <AlertTriangle className="w-5 h-5 text-theme animate-pulse" />
